  ...
  // DATATABLE AJAX
   var pemeriksaan_pasien = $('#pemeriksaan_pasien').on( 'processing.dt', function ( e, settings, processing ) {
        ...
        ajax: {
            ...
            type: 'POST',
            data: {"get_datatable_pemeriksaan":"", "id_poli":'<?= id_poli_dokter(userid()) ?>'}
        },
        // "columnDefs": [
   ...
   var riwayat_pasien = $('#riwayat_pasien').on( 'processing.dt', function ( e, settings, processing ) {
        ...
        ajax: {
            url: admin_url+'submit',
            type: 'POST',
            data: {"get_datatable_riwayat":"", "id_dokter":'<?= datadokter(userid(), 'id_dokter') ?>'}
        },
        // "columnDefs": [
        //     { className: "table-config", "targets": [5] }
        // ],
        ...
        buttons: [
            {
                ...
                title: 'Riwayat Pemeriksaan Dokter - <?= nama_dokter(userid()) ?>',
                messageTop: '<?= poli(id_poli_dokter(userid())) ?>'
            },
            {
                extend: 'print',
